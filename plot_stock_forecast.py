...
def build_actual_series(df_test: pd.DataFrame) -> pd.DataFrame:
    ...
    # Multiple frequencies can point to the same actual market date.
    # For the same date, the actual close should be the same or very close.
    actual = (
...
def build_predicted_series(df_test: pd.DataFrame) -> pd.DataFrame:
    ...
    # When multiple frequencies predict the same date, average them for a single whole-timeline view.
    predicted = (
